function CAddonWarsong:RespawnFlagForTeam(nTeam, vPos, options, soldier)
    ...
    if GetMapName() == "warsong" and soldier then
    ...
    if vPos then vSpawnPos = vPos end

    local vDestination = vSpawnPos
    if options and options.start_point then
...
function CAddonWarsong:RespawnFlagBoth(vPos, options)
    ...
    hIcon = self.flagIconUnits[DOTA_TEAM_GOODGUYS]
    if hIcon then
        hIcon:SetAbsOrigin(vDestination)
    end
...
function CAddonWarsong:RemindClients_FlagsRemaining()
    local radiantRemaining = self.nWinConditionGoal - self.nCapturedFlagsCount[DOTA_TEAM_GOODGUYS]
    local direRemaining = self.nWinConditionGoal - self.nCapturedFlagsCount[DOTA_TEAM_BADGUYS]
    CustomGameEventManager:Send_ServerToAllClients('update_flags_count', {
        radiant = radiantRemaining,
        dire = direRemaining
    })
...
function CAddonWarsong:DifferenceFlags()
    ...
    if difference >= FLAGS_DIFFERENCE_TIER_1 then 
        if not self.wasRewardFlagsTier1 or not self.wasRewardFlagsTier1[loser] then
            self.wasRewardFlagsTier1 = { [leader] = false, [loser] = true }
    ...
    DoWithAllPlayers(function(player, hero)
        if hero and hero.balanceModifier then
            ...
            if place == "last" and LAST_MODIFIER_BALANCE[tier] then
                ...
                hero.balanceModifier.incomingDamage = LAST_MODIFIER_BALANCE[tier].incoming or 0
                hero.balanceModifier.outgoingDamage = LAST_MODIFIER_BALANCE[tier].outgoing or 0
            else
