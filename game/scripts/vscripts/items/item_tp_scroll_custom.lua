...
function item_tp_scroll_custom:OnChannelFinish(bInterrupted)
    ...
    if not bInterrupted then 
        ...
        ParticleManager:DestroyParticle(self.endParticle, false)
    else 
        self:EndCooldown()
...
function item_tp_scroll_custom:OnSpellStart()
    ...
    self.viewer = AddFOWViewer(self:GetCaster():GetTeamNumber(), self.point, vision_radius, self:GetChannelTime(), false)
    ...
    ParticleManager:SetParticleControl(self.endParticle, 0, self.point)
    ParticleManager:SetParticleControl(self.endParticle, 1, self.point)
