LinkLuaModifier('modifier_item_flag_carrier_both', 'items/item_flag_both.lua', LUA_MODIFIER_MOTION_NONE)
...
local function DropFlag(vPos)
	local hFlag = GameRules.AddonTemplate:RespawnFlagBoth(vPos)
	hFlag.hReturnCountdown = FlagReturnCountdown(hFlag)
end

item_flag_both = class({
	GetOwnerTeam = function(self)
		if self.caster then
			return self.caster:GetTeamNumber()
		end
...
modifier_item_flag_carrier_both = class({
	...
	GetEffectName = function(self) 
		return 'particles/econ/items/necrolyte/necro_ti9_immortal/necro_ti9_immortal_ambient'
	end,
    ...
    OnCreated = function(self, keys)
		...
		self.nOwnerTeam = self:GetParent():GetTeamNumber()

		if self.nOwnerTeam == DOTA_TEAM_BADGUYS then
			self.nOwnerTeam = DOTA_TEAM_BADGUYS
			self.nOpponentTeam = DOTA_TEAM_GOODGUYS
		else
			self.nOwnerTeam = DOTA_TEAM_GOODGUYS
	...
	OnDestroy = function(self)
		...
		if self.bDelivered then
			...
			DropFlag(self.carrier:GetOrigin())
