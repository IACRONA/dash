modifier_donate_pet = modifier_donate_pet or class({})
...
function modifier_donate_pet:OnCreated()
	...
	-- ОПТИМИЗАЦИЯ FPS: Уменьшена частота обновления с 0.1 до 1.0 секунды
...
function modifier_donate_pet:GetModifierMoveSpeedOverride()
	return self.owner:GetMoveSpeedModifier(self.owner:GetBaseMoveSpeed(), false) * self.movespeed_multiplier
end


function modifier_donate_pet:GetModifierIgnoreMovespeedLimit() return 1 end


if not IsServer() then return end


function modifier_donate_pet:PlayBlinkParticle(from, to)
