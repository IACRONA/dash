...
function modifier_ability_upgrades_controller:GetModifierOverrideAbilitySpecialValue(params)
	...
	if upgrade_data.DisabledWithFacetID and upgrade_data.DisabledWithFacetID == self.parent:GetHeroFacetID() then
		-- print("2) disabled", special_value_name, "wrong facet", upgrade_data.DisabledWithFacetID, self.parent:GetHeroFacetID())
		return base_value
	end
